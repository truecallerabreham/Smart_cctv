...
const Index = () => {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      content: "SmartGuard online. Upload CCTV footage to begin incident analysis. You can ask me to find specific moments, retrieve clips, or answer questions about what's in the footage.",
      isUser: false,
      timestamp: new Date()
    }
  ]);
    ...
    const pollInterval = setInterval(() => {
      uploadedVideos.forEach(async (video) => {
        if (video.taskId && video.processingStatus === 'in_progress') {
          try {
            const response = await fetch(`/task-status/${video.taskId}?XTransformPort=8080`);
            if (response.ok) {
  ...
  const generateAIResponse = async (userMessage: string, fileUrl?: string, fileType?: 'image' | 'video'): Promise<{ message: string; clipPath?: string }> => {
    try {
      ...
      const response = await fetch('/chat?XTransformPort=8080', {
        method: 'POST',
        headers: {
  ...
  const handleVideoUpload = async (file: File) => {
    ...
    try {
      ...
      const uploadResponse = await fetch('/upload-video?XTransformPort=8080', {
        method: 'POST',
        body: formData,
      });
      ...
      const processResponse = await fetch('/process-video?XTransformPort=8080', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
