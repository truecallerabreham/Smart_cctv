...
if TYPE_CHECKING:
    from smartguard_mcp.video.ingestion.models import CachedTable
...
class VideoProcessor:
    ...
    def _add_audio_transcription(self):
        self.audio_chunks.add_computed_column(
            transcription=gemini_transcribe(
                audio_path=self.audio_chunks.audio_chunk,
            ),
            if_exists="ignore",
        )
    ...
    def _add_caption_embedding_index(self):
        self.frames_view.add_embedding_index(
            ...
            string_embed=sentence_transformer.using(model_id=settings.CAPTION_SIMILARITY_EMBD_MODEL),
            if_exists="replace_force",
        )
