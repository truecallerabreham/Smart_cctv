...
from smartguard_api.agent.base_agent import BaseAgent
from smartguard_api.agent.llm.llm_tool import transform_tool_definition
from smartguard_api.agent.memory import Memory, MemoryRecord
from smartguard_api.config import get_settings
from smartguard_api.models import (
...
class LLMAgent(BaseAgent):
    """Tool-use agent powered by an OpenAI-compatible LLM (Gemini 2.0 Flash).

    Mirrors the original Groq agent flow: routing -> tool-use -> general response,
    with MCP-discovered tools, conversation memory, and Opik tracing.
    """
    ...
    def __init__(
        ...
        # OpenAI-compatible client pointed at Gemini's OpenAI-compat endpoint.
        self.client = OpenAI(
            api_key=settings.LLM_API_KEY,
            base_url=settings.LLM_BASE_URL,
        )
        # JSON mode is the most reliable structured-output strategy across
        # OpenAI-compatible providers (incl. Gemini).
